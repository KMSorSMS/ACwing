...
int main()
{
	...
	for(int i = 0; i < input_str.size(); i++)
	{
		...
		if(in_str && ((input_str[i] == '"' && last_str_str == '"') ||
					  (input_str[i] == '\'' && last_str_str == '\'')))
		{
			// printf("unlock\n");
			in_str = false;
		}
		else if(!in_str && (input_str[i] == '"' || input_str[i] == '\''))
		{
			// printf("lock:%d\n", i);
			// printf("==%c==\n", input_str[i]);
			// // printf("%c%c%c%c%c\n",
		...
		if(in_str == false && input_str[i] == '*' && last_str == '/')
		{
			...
			i++;
			char tmp_last = ' ';
			while(input_str[i] != '*' || input_str[i + 1] != '/') { i++; }
			last_str = ' ';
			i++;
			// printf("\n===\n%s\n===\n", input_str.data() + i);
			// printf("\noutput is: %s\n", output_str.data());
