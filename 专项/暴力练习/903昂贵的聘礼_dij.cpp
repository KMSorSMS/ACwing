...
int dij(int up,int down){
    ...
    while(heap.size()){
        ...
        if(st[node]) continue;
        st[node] = true;
        if(node == 1) return len;
        for(int i=0;i<=n;i++){
            // printf("check i:%d,graph[node][i]:%d,dist[i]:%d node:%d\n",i,graph[node][i],dist[i],node);
            if(!st[i] && degrees[i]>=down && degrees[i] <= up && graph[node][i] != INF && dist[i] > dist[node]+graph[node][i] ) {
                dist[i] = dist[node] + graph[node][i];
                // printf("narrow:%d,node:%d,dist:%d\n",i,node,dist[i]);
                heap.push({dist[i],i});
            }
        }
    }
    printf("return from last\n");
    return dist[1];
}
...
int main(){
    ...
    int min_ = INF;
    for(int i=degrees[1] - m;i<=degrees[1];i++){
        int res = dij(i+m,i);
