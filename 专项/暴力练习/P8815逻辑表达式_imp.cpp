...
void pass_circle(int& i)
{
	char in = input_str[i];
	if(in == '(')
	{
		int count_circle = 1;
		while(count_circle != 0)
		{
			in = input_str[++i];
			if(in == ')')
				count_circle--;
...
int main()
{
	...
	for(int i = 0; i < input_str.size(); i++)
	{
		...
		else
		{
			...
			else if(in == '&' && nums.top() == 0)
			{

				count_and++;
				i++;
				in = input_str[i];
				// printf("before i:%d,in:%c\n", i, input_str[i]);
				if(in == '(') { pass_circle(i); }
				// printf("i:%d,in:%c\n", i, input_str[i]);
			}
			else if((in == '&' && !ops.empty() && ops.top() == '|') || in == '(') { ops.push(in); }
			else if(in == ')')
			{
				while(!ops.empty() && ops.top() != '(') { eval(); }
				ops.pop();
			}
			else
			{
